...
def attack_udr(model, X, y, epsilon, alpha, attack_iters, restarts,
    ...
    for _ in range(restarts):
        ...
        for _ in range(attack_iters):
            ...
            if early_stop:
                index = torch.where(robust_output.max(1)[1] == y)[0]
            else:
                index = slice(None,None,None)
            if not isinstance(index, slice) and len(index) == 0:
                break
            if mixup: 
                criterion = nn.CrossEntropyLoss()
                loss = mixup_criterion(criterion, model(normalize(X+delta)), y_a, y_b, lam)
            else: # mixup default is 0
                # loss = F.cross_entropy(output, y) #原paper beta=0
                loss = beta*F.cross_entropy(robust_output, y) + F.cross_entropy(output, y) # beta = 1.0
            loss.backward()
            grad = delta.grad.detach()
            d = delta[index, :, :, :]
            g = grad[index, :, :, :]
...
def get_args():
    ...
    parser.add_argument('--data-dir', default='/home/featurize/cifar-data', type=str)
...
def main():
    ...
    model_name = '{}ds=cifar10_model={}_method={}_mixloss'.format('/home/featurize/work/cifar10-resnet/',args.model, 'awp_at_udr')
    ...
    if args.model == 'PreActResNet18':
        ...
        # opt = torch.optim.SGD(params, lr=args.lr_max, momentum=0.9, weight_decay=3.5e-3)
    ...
    if args.lr_schedule == 'superconverge':
        ...
        def lr_schedule(t):
            ...
            if t < 75:
                ...
                return args.lr_max / 10000.
            else:
                return args.lr_max / 100000.
        # 40-0.01, 75=0.001, 90=0.0001, 100=0.00001 
    ...
    if args.resume:
        ...
        if os.path.exists(os.path.join(model_name, f'model_best.pth')):
            lamda = torch.load(os.path.join(model_name, f'model_best.pth'))['previous_lamda']
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_batches):
            ...
            # opt.zero_grad()
            # robust_loss.backward() # 原code
            # opt.step()
            mix_loss = beta*robust_loss + loss
            opt.zero_grad()
            mix_loss.backward() #修改为mixloss
            opt.step()
...
# !python train_cifar10_udr.py --model=PreActResNet18 --batch-size=128 --lr-max=0.01
